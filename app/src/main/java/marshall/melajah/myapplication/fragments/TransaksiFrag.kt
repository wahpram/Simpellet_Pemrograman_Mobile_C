...
import marshall.melajah.myapplication.DetailTransaksi
import marshall.melajah.myapplication.R
import marshall.melajah.myapplication.TransaksiAdapter
 ...
 * Use the [TransaksiFrag.newInstance] factory method to
...
class TransaksiFrag : Fragment() {
    ...
    private lateinit var transactionList: ArrayList<Transaksi>
...
//        visibilityOptions() //visibility option spinner
        ...
        transactionList = arrayListOf<Transaksi>()
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (snapshot.exists()){
                    when (selectedType) {
                        "All Type" -> { //all option selected
                            for (transactionSnap in snapshot.children){
                                val transactionData = transactionSnap.getValue(Transaksi::class.java) //reference data class
                    ...
                    if (transactionList.isEmpty()){ //if there is no data being displayed
                        ...
                        val mAdapter = TransaksiAdapter(transactionList)
                        transactionRecyclerView.adapter = mAdapter

                        mAdapter.setOnItemClickListener(object: TransaksiAdapter.onItemClickListener{ //item click listener and pass extra data
                            override fun onItemClick(position: Int) {
                                val intent = Intent(this@TransaksiFrag.activity, DetailTransaksi::class.java)
